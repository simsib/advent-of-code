...
// const input = `L8,D5,R5,U3
// D7,L6,U4,R4`;

const [line1, line2] = input.split('\n');

const line1Commands = line1.split(',').map(x => ({ dir: x.substring(0, 1), val: Number(x.substring(1)) }));

const line2Commands = line2.split(',').map(x => ({ dir: x.substring(0, 1), val: Number(x.substring(1)) }));
let findDuplicates = arr => arr.filter((item, index) => arr.indexOf(item) != index)

function lineToCoords(last, count, doX, doY) {
    const coords = [];
    for (let i = 1; i <= count; i++) {
        ...
        const {x, y} = element;
        coords.push({x, y, search: `${x}+${y}`});
    }
    return coords;
}

function plus(cord, modifier) {
    return cord + modifier;
}

function minus(cord, modifier) {
    return cord - modifier
}
...
const commandReducer = (coords, action) => {
    const last = coords[coords.length - 1];
    const {x, y} = getCordFunctions(action.dir);
    coords.push(...lineToCoords(last, action.val, x, y));

    return coords;
}


const line1Coords = line1Commands.reduce(commandReducer, [{ x: 0, y: 0 }]);
const line2Coords = line2Commands.reduce(commandReducer, [{ x: 0, y: 0 }]);
const allCoords = [
    ...Array.from(new Set(line1Coords.map(x => x.search))),
    ...Array.from(new Set(line2Coords.map(x => x.search)))]
    .sort();

const results = [];
for (let i = 0; i < allCoords.length - 1; i++) {
    if (allCoords[i + 1] == allCoords[i]) {
        results.push(allCoords[i]);
    }
}
const result = results
        .filter(x => x)
        .map((cord) => {
                const coords = cord.split('+');
        ...
        .map((cord) => {
            ...
            const a1 = line1Coords.findIndex(l => x === l.x && y === l.y);
            const a2 = line2Coords.findIndex(l => x === l.x && y === l.y);
            return {
                x,
                y,
                distance,
        ...
        .sort((a, b) => { return a.shortest - b.shortest});
